pplfunction output = pplane2017b(action,input1,input2,input3)

%   pplane2017b  is an interactive tool for studying planar autonomous systems of
%	differential equations.  When pplane2017b is executed, a pplane2017b Setup
...
%
%	When the Proceed button is pressed on the Setup window, the pplane2017b
...
%	the pplane2017b Display window, the solution to the system with that
...
% Updated for MATLAB 2017b/2018a by Marianna Wrzos-Kaminska,
% following solution suggestion posted online by Dan Schult

startstr = 'pplane2017b';
if nargin < 1
  action ='initialize';
end
...
if strcmp(action,'initialize')
   ...
   % First we make sure that there is no other copy of pplane2017b
   % running, since this causes problems.	
   pph = findobj('tag','pplane2017b');
   if ~isempty(pph);
      qstring = {'There are some pplane2017b figures open although they may be invisible.  ';...
            'What do you want to do?'};
      tstring = 'Only one copy of pplane2017b can be open at one time.';
      b1str = 'Restart pplane2017b.';
   ...
   % Next we look for old files created by pplane2017b.
   ...
   for k = 1:length(oldfiles)
      ...
      ll = fgetl(fid);
      fclose(fid);
      if strcmp(ll,'%% Created by pplane2017b')
         delete(fn)
      else
	l= length(ofiles);
	ofiles{l+1} = fn;
  ...
  ud.color = color;
  ud.level = ' ';
  ppset = figure('name','pplane2017b Setup','NumberTitle','off',...
		 'tag','pplane2017b','visible','off',...
		 'UserData',ud);
  
  pplane2017b('figdefault',ppset);
  ...
  butt(2) = uicontrol('style','push',...
		      'pos',rwind,...
		      'string','Revert',...
		      'CallBack','pplane2017b(''revert'')',...
		      'visible','off');
  
  butt(3) = uicontrol('style','push',...
		      'pos',pwind,...
		      'string','Proceed',...
		      'CallBack','pplane2017b(''proceed'')',...
		      'visible','off');
  ...
  callnfpts = [
      'ppset = findobj(''name'',''pplane2017b Setup'');',...
      'ud = get(ppset,''user'');'...
      'me = ud.h.npts;',...
  ...
  meload = uimenu(mefile,'label','Load a system ...',...
		  'CallBack','pplane2017b(''loadsyst'',''system'');',...
		  'pos',1);
  mesave = uimenu(mefile,'label','Save the current system ...',...
		  'CallBack','pplane2017b(''savesyst'',''system'');',...
		  'pos',2);
  meloadg = uimenu(mefile,'label','Load a gallery ...',...
		   'CallBack','pplane2017b(''loadsyst'',''gallery'');',...
		   'separator','on','pos',3);
  mesaveg = uimenu(mefile,'label','Save a gallery ...',...
		   'CallBack','pplane2017b(''savesyst'',''gallery'');',...
		   'tag','savegal','pos',4);
  ...
  melddg = uimenu(mefile,'label','Load the default gallery',...
		  'CallBack','pplane2017b(''loadsyst'',''default'');',...
		  'enable','on',...
  ...
  merestart = uimenu(mefile,'label',...
		     'Restart pplane2017b',...
		     'CallBack','pplane2017b(''restart'')',...
		     'separator','on','pos',12);
  
  mequit = uimenu(mefile,...
		  'label','Quit pplane2017b',...
		  'CallBack','pplane2017b(''quit'')',...
  ...
  % Gallery menu.
   ...
   meadd = uimenu(sysmenu,'label','Add current system to the gallery',...
      'CallBack','pplane2017b(''addgall'');','tag','add system');
   sep = 'on';
   for kk = 1:length(system)
      kkk = num2str(kk);
      if kk == 2, sep = 'off';end
      sysmen(kk) = uimenu(sysmenu,'label',system(kk).name,...
         'CallBack',['pplane2017b(''system'',',kkk,')'],...
...
elseif strcmp(action,'loadsyst')  % This loads either a system or a gallery.
   ...
   for j = 1:nnn
      ...
      newsyst.name = sname;
      ignore = pplane2017b('addgall',newsyst);
      if ignore == -1;
   ...
   if strcmp(type,'system') % Added a system.
      ...
      pplane2017b('system',kk);
   end  
   if strcmp('type','default')
      pplane2017b('system',1);
   end  
   set(sud.h.gallery,'enable','on');
   x = 1;
   xp = [xp(2),x,x,xp(2)];
...
elseif strcmp(action,'proceed')
   ...
   sud.o = sud.c;
   set(ppset,'user',sud);
   he = findobj('name','pplane2017b Equilibrium point data');
   hl = findobj('name','pplane2017b Linearization');
   close([he;hl]);
   % set([he;hl],'vis','off');
   ...
   if (sud.flag == 1)
     ...
     NumbFPts = sud.c.npts;
     ppdisp = findobj('name','pplane2017b Display');
     dud = get(ppdisp,'user');
    ...
    %  set([arr.hx;arr.hy;arr.barrows],'vis','on');
	 ...
	 if isempty(pv) 
	   ...
	   if isempty(str2num(pv)) % This is an expression.
	     ...
	     if (err | isempty(pval))
	       ...
	       fprintf('\a')
	       errordlg(errmsg,'pplane2017b error','on');
	       warning(SS)
	       return
	     end
	   end
	   xxderivstr = pplane2017b('paraeval',pn,pv,xderivstr);
	   yyderivstr = pplane2017b('paraeval',pn,pv,yderivstr);
     ...
     commstr = '%%%% Created by pplane2017b\n\n';
     ...
     % Find pplane2017b Display if it exists.	
     % If pplane2017b Display exists, update it.  If it does not build it.
	 ...
	 if ~isempty(menull)
      ...
      ppdisp = figure('name','pplane2017b Display',...
		       'NumberTitle','off',...
		       'Interruptible','on',...
		       'visible','off',...
		       'tag','pplane2017b');
       pplane2017b('figdefault',ppdisp);
       dud = get(ppdisp,'user');
       dud.syst = sud.c;
	...
	otherwise
	 ...
	 set(gcf,'defaultuicontrolfontsize',fs*0.9);
       ...
       dbutt(2) = uicontrol('style','push',...
			    ...
			    'string','Quit',...
			    'CallBack','pplane2017b(''quit'')','visible','off');
       
       dbutt(3) = uicontrol('style','push',...
			    'pos',[buttl,ppaxb+buttsep,buttw,butth],...
			    'string','Print',...
			    'CallBack','pplane2017b(''print'')','visible','off');	
       ...
       merestart = uimenu(fmenu,'label',...
			  ...
			  'CallBack','pplane2017b(''restart'')',...
			  'separator','on');
       mequit = uimenu(fmenu,'label','Quit pplane2017b',...
		       'CallBack','pplane2017b(''quit'')','separator','off');
       ...
       menu(3) = uimenu(megraph,'label',[txname,' vs. t'],...
			'CallBack','pplane2017b(''plotxy'',1)');
       
       menu(5) = uimenu(megraph,'label',[tyname,' vs. t'],...
			'CallBack','pplane2017b(''plotxy'',2)');
       
       meplot3 = uimenu(megraph,'label','Both',...
			'CallBack','pplane2017b(''plotxy'',3)');
       
       meplot4 = uimenu(megraph,'label','3 D',...
			'CallBack','pplane2017b(''plotxy'',4)');
       
       meplot5 = uimenu(megraph,'label','Composite',...
			'CallBack','pplane2017b(''plotxy'',5)');
	 ...
	 metest = uimenu(solmenu,'label','Test case',...
			 'CallBack','pplane2017b(''test case'')',...
	...
	ode113ch = 'off';
	 ...
	 ode23tbch = 'off';
       ...
       menu(6) = uimenu(menu(1),'label','Make the Display Window inactive.',...
			'CallBack','pplane2017b(''hotcold'')','separator','on');
	   ...
	   'CallBack','pplane2017b(''showbar'')');
       
       set(0,'showhiddenhandles',hhsetup);
       
       set(gcf,'WindowButtonDownFcn','pplane2017b(''down'')');
       set(ppdisp,'WindowButtonMotionFcn','pplane2017b(''cdisp'')');
	...
	case 6
     ...
     ppkbd = findobj('name','pplane2017b Keyboard input','vis','on');
     if ~isempty(ppkbd),pplane2017b('kbd'),end
     pplevel = findobj('name','pplane2017b Level sets');
...
elseif strcmp(action,'linear')
   ...
   % Get the information from pplane2017b Equilibrium ...
   ...
   % Find pplane2017b Linearization if it exists.	
   % If pplane2017b Linearization exists, update it.  If it does not build it.
   
   pplin= findobj('name','pplane2017b Linearization');
   if (~isempty(pplin))
      figure(pplin);
      dud = get(pplin,'user');
      dud.syst = system;
      ...
      pplin = figure('name','pplane2017b Linearization',...
         'interrupt','on',...
         'visible','off',...
         'tag','pplane2017b');
      pplane2017b('figdefault',pplin);
      dud = get(pplin,'user');
      dud.dir = 0;
      dud.syst = system;
      dud.settings = settings;
      ...
      mequit = uimenu(fmenu,'label','Quit pplane2017b',...
	  'CallBack','pplane2017b(''quit'')','separator','on','pos',9);
	...
	chkd = 'on';
      ...
      set(pplin,'WindowButtonDownFcn',['pplane2017b(''down'',',num2str(pplin),')']);
...
elseif strcmp(action,'dirfield')
   ...
   % Find pplane2017b Display and get the user data.
   ...
   N = dud.syst.npts;
   if strcmp(get(disph,'name'),'pplane2017b Linearization') 
      N = floor(3*N/4);
   end    
...
elseif strcmp(action,'hotcold')
   ...
   if (findstr(get(mehc,'label'),'inactive'))
      ...
      set(ppdisp,'WindowButtonDownFcn','pplane2017b(''down'')');
...
elseif strcmp(action,'down')
  ...
  pplane2017b('solution',initpt,disph);
...
elseif strcmp(action,'test case')
   ...
   for k = 1:10
      initpt = k*[-.2 .2];
      pplane2017b('solution',initpt,ppdisp);
      initpt = k*[-.2 -.2];
      pplane2017b('solution',initpt,ppdisp);
   end  
   pplane2017b('solution',[1,-1],ppdisp);
   dud = get(gcf,'user');
   nstr = get(dud.notice,'string');
   nstr(1:4) = nstr(2:5);
   nstr{5,1} = 'Ready.';
...
elseif strcmp(action,'kcompute')
   ...
   % button on the pplane2017b Keyboard figure.
   
   compute = 1;
   kh = get(gcf,'user');
   ppdisp = kh.fig;
   if (isempty(ppdisp))
      pplane2017b('confused');
   end
   dud = get(ppdisp,'user');
   ppdispa = dud.axes;
   aud = get(ppdispa,'user');
   ...
   for kk = 1:6;
	 ...
	 xstr = pplane2017b('paraeval',pn,pv,xstr);
	 ystr = pplane2017b('paraeval',pn,pv,ystr);
       end
     end
   end
   xvalue = str2num(xstr);
...
elseif strcmp(action,'kbd')
   ...
   % sets up the pplane2017b Keyboard figure which allows accurate input of 
   ...
   ppkbd = findobj('name','pplane2017b Keyboard input');
   if  ~isempty(ppkbd)
     delete(ppkbd);
   end
         
   ppkbd = figure('name','pplane2017b Keyboard input',...
		  'vis','off',...
		  'tag','pplane2017b');
   
   pplane2017b('figdefault',ppkbd);
   ...
   kbd.comp = uicontrol('style','push',...		
			'string','Compute','CallBack','pplane2017b(''kcompute'')');
...
%  z0 = ppginput(1);
  ...
  ppeqpt = findobj('name','pplane2017b Equilibrium point data');
  
   if (isempty(ppeqpt))
     ppeqpt = figure('vis','off','resize','on',...
 		    'name','pplane2017b Equilibrium point data',...
 		    'tag','pplane2017b');
     pplane2017b('figdefault',ppeqpt);
...
elseif strcmp(action,'zoomin')
   ...
   set(gcf,'WindowButtonDownFcn','pplane2017b(''zoom'')',...
...
elseif strcmp(action,'zoom')
   ...
   set(axh,'user',aud);	
   set(disph,'user',dud);
   set(disph,'WindowButtonDownFcn','pplane2017b(''down'')',...
      'WindowButtonUpFcn','');
   pplane2017b('dirfield',disph);
   ppset = findobj('name','pplane2017b Setup');
   if isempty(ppset)
      pplane2017b('confused');
   else
...
elseif strcmp(action,'showbar')
  ...
  if strcmp(state,'figure')
	...
	'clickedcallback','pplane2017b(''print'');');
    switch name
     case 'pplane2017b Display'
      set(findobj(sbfig,'tooltipstr','Zoom Out'),...
	  'clickedcallback',['pplane2017b(''zoomback'');' fixtb]);
      set(findobj(sbfig,'tooltipstr','Zoom In'),...
	  'clickedcallback',['pplane2017b(''zoomin'');' fixtb]);
      delete(findobj(sbfig,'tooltipstr','Rotate 3D'));
      
     case 'pplane2017b Linearization'
...
elseif strcmp(action,'settings')
   ...
   % up the pplane2017b Settings window, which allows the user to interactively
   ...
   setfig = findobj('name','pplane2017b Settings');
   if ~isempty(setfig);
     delete(setfig);
   end
   
   setfig = figure('name','pplane2017b Settings',...
		   'tag','pplane2017b','vis','off');
   
   pplane2017b('figdefault',setfig);
   ...
   chb = uicontrol('style','push',...
      ...
      'CallBack','pplane2017b(''setchange'')');
...
elseif strcmp(action,'setchange')
   ...
   % on the pplane2017b Settings window.
...
elseif strcmp(action,'plotxy')  % Start a graph 
  ...
  x=get(objh,'xdata');
  y=get(objh,'ydata');
  hh = pplane2017b('plotxyfig',type,gcf);
  set(ppdisp,'WindowButtonDownFcn',oldcall);
  aud = get(hh(2),'user');
  ud = get(hh(1),'user');
  ud.data = [t;x;y];
  set(hh(1),'user',ud);
  pplane2017b('plxy',aud.rad,hh(1));
...
elseif strcmp(action,'plotxyfig')  % Build the graph figure.
  ...
  ppxy = figure('NumberTitle','off',...
		'tag','pplane2017b','visible','off');
  %name = ['pplane2017b t-plot # ',int2str(ppxy)];
  name = ['pplane2017b t-plot # ',int2str(get(ppxy,'Number'))];
  ud.color = dud.color;
  ud.type = 0;
  set(ppxy,'name',name,'user',ud);
  pplane2017b('figdefault',ppxy);
  ...
  set(findobj(fmenu,'label','Print Pre&view...'),'pos',6);
  set(findobj(fmenu,'label','&Print...'),'pos',7);
  qmenu = uimenu(fmenu,'label','Quit pplane2017b',...
		 'CallBack','pplane2017b(''quit'')',...
		 'separator','on');
...
elseif strcmp(action,'plxy')
   ...
   switch type
    case 1 
      ...
      ylabel(xstring)
      set(r3dh,'vis','off')
      set(fig,'WindowButtonDownFcn','pplane2017b(''plxybdf'')');
    case 2
...
elseif strcmp(action,'crop')
   ...
   hh = pplane2017b('plotxyfig',type,ppxy);
   ud = get(hh(1),'user');
   ud.data = [t;x;y];
   set(hh(1),'user',ud);
   aud = get(hh(2),'user');
...
elseif strcmp(action,'plxybdf')
   ...
   dispa = gca;
   dispf = gcf;
   set(dispf,'windowbuttonmotionfcn','pplane2017b(''plxybmf'')',...
      'windowbuttonupfcn','pplane2017b(''plxybuf'')',...
      'inter','on');
   aud = get(dispa,'user');
   delete(aud.h);
   aud.h = [];
...
elseif strcmp(action,'print')
   ...
   nstr = get(dud.notice,'string');
   nstr(1:4) = nstr(2:5);
   nstr{5} = 'Preparing to print the pplane2017b Display Window.  Please be patient.';
   set(dud.notice,'string',nstr);
   
   nstr(1:4) = nstr(2:5);
   nstr{5} = 'Printing the pplane2017b Display Window.';
   set(dud.notice,'string',nstr);
...
elseif strcmp(action,'figdefault')
  ...
  fig = input1;
  set(fig,'CloseRequestFcn','pplane2017b(''closefcn'')');
  ppset = findobj('name','pplane2017b Setup');
  sud = get(ppset,'user');
  ud = get(fig,'user');
  ud.ssize = sud.ssize;
...
elseif strcmp(action,'zoominsq')
   ...
   set(gcf,'WindowButtonDownFcn','pplane2017b(''zoomsqd'')',...
      'WindowButtonUpFcn','1;','inter','on');
   set(gca,'inter','on');
   dud = get(gcf,'user');
   nstr = get(dud.notice,'string');
...
elseif strcmp(action,'zoomsqd')
   ...
   set(ppdispa,'user',aud);
   set(gcf,'windowbuttonmotionfcn','pplane2017b(''zoomsqm'')',...
      'windowbuttonupfcn','pplane2017b(''zoomsqu'')');
...
elseif strcmp(action,'zoomsqu')
   ...
   set(disph,'user',dud);
   set(disph,'WindowButtonDownFcn','pplane2017b(''down'')',...
      'WindowButtonMotionFcn','pplane2017b(''cdisp'')',...
      'WindowButtonUpFcn','');
   pplane2017b('dirfield',disph);
   ppset = findobj('name','pplane2017b Setup');
   if isempty(ppset)
...
elseif strcmp(action,'level')
  ...
  pplevel = figure('name','pplane2017b Level sets',...
		 'vis','off',...
		 'NumberTitle','off','tag','pplane2017b');
  
  pplane2017b('figdefault',pplevel);
...
elseif strcmp(action,'levcomp')
  ...
  for kk = 1:4;
	...
	pv(find(abs(pv)==32))=[];
	lfcn = pplane2017b('paraeval',pn,pv,lfcn);
      end
    end
  end
  l = length(lfcn);
...
elseif strcmp(action,'closefcn')
   ...
   fig = gcf;  
   name = get(fig,'name');
   if strcmp(name,'pplane2017b Setup') | strcmp(name,'pplane2017b Display')
      quest = ['Closing this window will cause all pplane2017b ',...
            'windows to close, and pplane2017b will stop.  ',...
            'Do you want to quit pplane2017b?'];
      butt = questdlg(quest,'Quit pplane2017b?','Quit','Cancel','Quit');
      if strcmp(butt,'Quit')
         pplane2017b('quit');
      end
   elseif strcmp(name,'pplane2017b Linearization')
      dud = get(fig,'user');
      fcn = dud.function;
...
elseif strcmp(action,'confused')
   ...
   tstring = 'pplane2017b is totally confused';
   qstring = {['You will have to restart pplane2017b from '...
      ...
      'What do you want to do?'};
   bstr1 = 'Quit and restart pplane2017b.';
   bstr2 = 'Just quit pplane2017b.';
   bstr3 = 'Do nothing.';
   answer = questdlg(qstring,tstring,bstr1,bstr2,bstr3,bstr1);
   if strcmp(answer,bstr1)
      delete(findobj('tag','pplane2017b'));
      pplane2017b;return
...
% PPOUT  is the output function for pplane2017b.

%  Copywright (c)  John C. Polking, Rice University
%  Last Modified: January 21, 2001

output = 0;
ppdisp = findobj(get(0,'child'),'flat','name','pplane2017b Display'); %gca;   % 
dud = get(ppdisp,'user');
ppdispa = dud.axes;
dfcn = dud.function;
ud = get(ppdispa,'user');
